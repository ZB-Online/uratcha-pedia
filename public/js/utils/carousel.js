...
export function renderSearchedMovieCarousel({ $target, initialState }) {
  const $searchMovieCarousel = document.createElement('div');
  $target.appendChild($searchMovieCarousel);
  ...
  this.render = () => {
    ...
    $searchMovieCarousel.innerHTML =
      `<ul class="search-result-container__list carousel-slides">` +
      movies
        .map(
          movie => `
