...
const signup = (req, res) => {
  ...
  if (userService.findUserByEmail(signupUser.email)) res.status(400).json(resData.successFalse(resMessage.EMAIL_ALREADY_EXIST));

  if (userService.findUserByName(signupUser.username)) res.status(400).json(resData.successFalse(resMessage.NAME_ALREADY_EXIST));
